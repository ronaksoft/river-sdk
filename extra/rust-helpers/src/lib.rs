...
use std::ops::{BitAnd, Sub, Add, BitAndAssign, Shr, Div};

use num_bigint::{BigInt, RandBigInt, Sign};
use num_bigint::prelude::ThreadRng;
...
fn SplitPQ(pq: BigInt) -> (BigInt, BigInt) {
    ...
    while !(g.cmp(&value1) == Greater && g.cmp(&what) == Less) {
        let q = rng.gen_bigint_range(zero(), &rndMax);
        q.bitand(&value15);
        q.bitand(&value17);
