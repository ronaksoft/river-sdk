// Code generated by easyjson for marshaling/unmarshaling. DO NOT EDIT.

package riversdk

import (
	json "encoding/json"
	easyjson "github.com/mailru/easyjson"
	jlexer "github.com/mailru/easyjson/jlexer"
	jwriter "github.com/mailru/easyjson/jwriter"
)

// suppress unused package warning
var (
	_ *json.RawMessage
	_ *jlexer.Lexer
	_ *jwriter.Writer
	_ easyjson.Marshaler
)

func easyjson27a961dcDecodeGitRonaksoftComRiverSdkSdkPrime(in *jlexer.Lexer, out *ServerKeys) {
	isTopLevel := in.IsStart()
	if in.IsNull() {
		if isTopLevel {
			in.Consumed()
		}
		in.Skip()
		return
	}
	in.Delim('{')
	for !in.IsDelim('}') {
		key := in.UnsafeFieldName(false)
		in.WantColon()
		if in.IsNull() {
			in.Skip()
			in.WantComma()
			continue
		}
		switch key {
		case "PublicKeys":
			if in.IsNull() {
				in.Skip()
				out.PublicKeys = nil
			} else {
				in.Delim('[')
				if out.PublicKeys == nil {
					if !in.IsDelim(']') {
						out.PublicKeys = make([]PublicKey, 0, 2)
					} else {
						out.PublicKeys = []PublicKey{}
					}
				} else {
					out.PublicKeys = (out.PublicKeys)[:0]
				}
				for !in.IsDelim(']') {
					var v1 PublicKey
					(v1).UnmarshalEasyJSON(in)
					out.PublicKeys = append(out.PublicKeys, v1)
					in.WantComma()
				}
				in.Delim(']')
			}
		case "DHGroups":
			if in.IsNull() {
				in.Skip()
				out.DHGroups = nil
			} else {
				in.Delim('[')
				if out.DHGroups == nil {
					if !in.IsDelim(']') {
						out.DHGroups = make([]DHGroup, 0, 2)
					} else {
						out.DHGroups = []DHGroup{}
					}
				} else {
					out.DHGroups = (out.DHGroups)[:0]
				}
				for !in.IsDelim(']') {
					var v2 DHGroup
					(v2).UnmarshalEasyJSON(in)
					out.DHGroups = append(out.DHGroups, v2)
					in.WantComma()
				}
				in.Delim(']')
			}
		default:
			in.SkipRecursive()
		}
		in.WantComma()
	}
	in.Delim('}')
	if isTopLevel {
		in.Consumed()
	}
}
func easyjson27a961dcEncodeGitRonaksoftComRiverSdkSdkPrime(out *jwriter.Writer, in ServerKeys) {
	out.RawByte('{')
	first := true
	_ = first
	{
		const prefix string = ",\"PublicKeys\":"
		out.RawString(prefix[1:])
		if in.PublicKeys == nil && (out.Flags&jwriter.NilSliceAsEmpty) == 0 {
			out.RawString("null")
		} else {
			out.RawByte('[')
			for v3, v4 := range in.PublicKeys {
				if v3 > 0 {
					out.RawByte(',')
				}
				(v4).MarshalEasyJSON(out)
			}
			out.RawByte(']')
		}
	}
	{
		const prefix string = ",\"DHGroups\":"
		out.RawString(prefix)
		if in.DHGroups == nil && (out.Flags&jwriter.NilSliceAsEmpty) == 0 {
			out.RawString("null")
		} else {
			out.RawByte('[')
			for v5, v6 := range in.DHGroups {
				if v5 > 0 {
					out.RawByte(',')
				}
				(v6).MarshalEasyJSON(out)
			}
			out.RawByte(']')
		}
	}
	out.RawByte('}')
}

// MarshalJSON supports json.Marshaler interface
func (v ServerKeys) MarshalJSON() ([]byte, error) {
	w := jwriter.Writer{}
	easyjson27a961dcEncodeGitRonaksoftComRiverSdkSdkPrime(&w, v)
	return w.Buffer.BuildBytes(), w.Error
}

// MarshalEasyJSON supports easyjson.Marshaler interface
func (v ServerKeys) MarshalEasyJSON(w *jwriter.Writer) {
	easyjson27a961dcEncodeGitRonaksoftComRiverSdkSdkPrime(w, v)
}

// UnmarshalJSON supports json.Unmarshaler interface
func (v *ServerKeys) UnmarshalJSON(data []byte) error {
	r := jlexer.Lexer{Data: data}
	easyjson27a961dcDecodeGitRonaksoftComRiverSdkSdkPrime(&r, v)
	return r.Error()
}

// UnmarshalEasyJSON supports easyjson.Unmarshaler interface
func (v *ServerKeys) UnmarshalEasyJSON(l *jlexer.Lexer) {
	easyjson27a961dcDecodeGitRonaksoftComRiverSdkSdkPrime(l, v)
}
func easyjson27a961dcDecodeGitRonaksoftComRiverSdkSdkPrime1(in *jlexer.Lexer, out *RiverConnection) {
	isTopLevel := in.IsStart()
	if in.IsNull() {
		if isTopLevel {
			in.Consumed()
		}
		in.Skip()
		return
	}
	in.Delim('{')
	for !in.IsDelim('}') {
		key := in.UnsafeFieldName(false)
		in.WantColon()
		if in.IsNull() {
			in.Skip()
			in.WantComma()
			continue
		}
		switch key {
		case "AuthID":
			out.AuthID = int64(in.Int64())
		case "AuthKey":
			if in.IsNull() {
				in.Skip()
			} else {
				copy(out.AuthKey[:], in.Bytes())
			}
		case "UserID":
			out.UserID = int64(in.Int64())
		case "Username":
			out.Username = string(in.String())
		case "Phone":
			out.Phone = string(in.String())
		case "FirstName":
			out.FirstName = string(in.String())
		case "LastName":
			out.LastName = string(in.String())
		case "Bio":
			out.Bio = string(in.String())
		case "Version":
			out.Version = int(in.Int())
		default:
			in.SkipRecursive()
		}
		in.WantComma()
	}
	in.Delim('}')
	if isTopLevel {
		in.Consumed()
	}
}
func easyjson27a961dcEncodeGitRonaksoftComRiverSdkSdkPrime1(out *jwriter.Writer, in RiverConnection) {
	out.RawByte('{')
	first := true
	_ = first
	{
		const prefix string = ",\"AuthID\":"
		out.RawString(prefix[1:])
		out.Int64(int64(in.AuthID))
	}
	{
		const prefix string = ",\"AuthKey\":"
		out.RawString(prefix)
		out.Base64Bytes(in.AuthKey[:])
	}
	{
		const prefix string = ",\"UserID\":"
		out.RawString(prefix)
		out.Int64(int64(in.UserID))
	}
	{
		const prefix string = ",\"Username\":"
		out.RawString(prefix)
		out.String(string(in.Username))
	}
	{
		const prefix string = ",\"Phone\":"
		out.RawString(prefix)
		out.String(string(in.Phone))
	}
	{
		const prefix string = ",\"FirstName\":"
		out.RawString(prefix)
		out.String(string(in.FirstName))
	}
	{
		const prefix string = ",\"LastName\":"
		out.RawString(prefix)
		out.String(string(in.LastName))
	}
	{
		const prefix string = ",\"Bio\":"
		out.RawString(prefix)
		out.String(string(in.Bio))
	}
	{
		const prefix string = ",\"Version\":"
		out.RawString(prefix)
		out.Int(int(in.Version))
	}
	out.RawByte('}')
}

// MarshalJSON supports json.Marshaler interface
func (v RiverConnection) MarshalJSON() ([]byte, error) {
	w := jwriter.Writer{}
	easyjson27a961dcEncodeGitRonaksoftComRiverSdkSdkPrime1(&w, v)
	return w.Buffer.BuildBytes(), w.Error
}

// MarshalEasyJSON supports easyjson.Marshaler interface
func (v RiverConnection) MarshalEasyJSON(w *jwriter.Writer) {
	easyjson27a961dcEncodeGitRonaksoftComRiverSdkSdkPrime1(w, v)
}

// UnmarshalJSON supports json.Unmarshaler interface
func (v *RiverConnection) UnmarshalJSON(data []byte) error {
	r := jlexer.Lexer{Data: data}
	easyjson27a961dcDecodeGitRonaksoftComRiverSdkSdkPrime1(&r, v)
	return r.Error()
}

// UnmarshalEasyJSON supports easyjson.Unmarshaler interface
func (v *RiverConnection) UnmarshalEasyJSON(l *jlexer.Lexer) {
	easyjson27a961dcDecodeGitRonaksoftComRiverSdkSdkPrime1(l, v)
}
func easyjson27a961dcDecodeGitRonaksoftComRiverSdkSdkPrime2(in *jlexer.Lexer, out *PublicKey) {
	isTopLevel := in.IsStart()
	if in.IsNull() {
		if isTopLevel {
			in.Consumed()
		}
		in.Skip()
		return
	}
	in.Delim('{')
	for !in.IsDelim('}') {
		key := in.UnsafeFieldName(false)
		in.WantColon()
		if in.IsNull() {
			in.Skip()
			in.WantComma()
			continue
		}
		switch key {
		case "N":
			out.N = string(in.String())
		case "FingerPrint":
			out.FingerPrint = int64(in.Int64())
		case "E":
			out.E = uint32(in.Uint32())
		default:
			in.SkipRecursive()
		}
		in.WantComma()
	}
	in.Delim('}')
	if isTopLevel {
		in.Consumed()
	}
}
func easyjson27a961dcEncodeGitRonaksoftComRiverSdkSdkPrime2(out *jwriter.Writer, in PublicKey) {
	out.RawByte('{')
	first := true
	_ = first
	{
		const prefix string = ",\"N\":"
		out.RawString(prefix[1:])
		out.String(string(in.N))
	}
	{
		const prefix string = ",\"FingerPrint\":"
		out.RawString(prefix)
		out.Int64(int64(in.FingerPrint))
	}
	{
		const prefix string = ",\"E\":"
		out.RawString(prefix)
		out.Uint32(uint32(in.E))
	}
	out.RawByte('}')
}

// MarshalJSON supports json.Marshaler interface
func (v PublicKey) MarshalJSON() ([]byte, error) {
	w := jwriter.Writer{}
	easyjson27a961dcEncodeGitRonaksoftComRiverSdkSdkPrime2(&w, v)
	return w.Buffer.BuildBytes(), w.Error
}

// MarshalEasyJSON supports easyjson.Marshaler interface
func (v PublicKey) MarshalEasyJSON(w *jwriter.Writer) {
	easyjson27a961dcEncodeGitRonaksoftComRiverSdkSdkPrime2(w, v)
}

// UnmarshalJSON supports json.Unmarshaler interface
func (v *PublicKey) UnmarshalJSON(data []byte) error {
	r := jlexer.Lexer{Data: data}
	easyjson27a961dcDecodeGitRonaksoftComRiverSdkSdkPrime2(&r, v)
	return r.Error()
}

// UnmarshalEasyJSON supports easyjson.Unmarshaler interface
func (v *PublicKey) UnmarshalEasyJSON(l *jlexer.Lexer) {
	easyjson27a961dcDecodeGitRonaksoftComRiverSdkSdkPrime2(l, v)
}
func easyjson27a961dcDecodeGitRonaksoftComRiverSdkSdkPrime3(in *jlexer.Lexer, out *DHGroup) {
	isTopLevel := in.IsStart()
	if in.IsNull() {
		if isTopLevel {
			in.Consumed()
		}
		in.Skip()
		return
	}
	in.Delim('{')
	for !in.IsDelim('}') {
		key := in.UnsafeFieldName(false)
		in.WantColon()
		if in.IsNull() {
			in.Skip()
			in.WantComma()
			continue
		}
		switch key {
		case "Prime":
			out.Prime = string(in.String())
		case "Gen":
			out.Gen = int32(in.Int32())
		case "FingerPrint":
			out.FingerPrint = int64(in.Int64())
		default:
			in.SkipRecursive()
		}
		in.WantComma()
	}
	in.Delim('}')
	if isTopLevel {
		in.Consumed()
	}
}
func easyjson27a961dcEncodeGitRonaksoftComRiverSdkSdkPrime3(out *jwriter.Writer, in DHGroup) {
	out.RawByte('{')
	first := true
	_ = first
	{
		const prefix string = ",\"Prime\":"
		out.RawString(prefix[1:])
		out.String(string(in.Prime))
	}
	{
		const prefix string = ",\"Gen\":"
		out.RawString(prefix)
		out.Int32(int32(in.Gen))
	}
	{
		const prefix string = ",\"FingerPrint\":"
		out.RawString(prefix)
		out.Int64(int64(in.FingerPrint))
	}
	out.RawByte('}')
}

// MarshalJSON supports json.Marshaler interface
func (v DHGroup) MarshalJSON() ([]byte, error) {
	w := jwriter.Writer{}
	easyjson27a961dcEncodeGitRonaksoftComRiverSdkSdkPrime3(&w, v)
	return w.Buffer.BuildBytes(), w.Error
}

// MarshalEasyJSON supports easyjson.Marshaler interface
func (v DHGroup) MarshalEasyJSON(w *jwriter.Writer) {
	easyjson27a961dcEncodeGitRonaksoftComRiverSdkSdkPrime3(w, v)
}

// UnmarshalJSON supports json.Unmarshaler interface
func (v *DHGroup) UnmarshalJSON(data []byte) error {
	r := jlexer.Lexer{Data: data}
	easyjson27a961dcDecodeGitRonaksoftComRiverSdkSdkPrime3(&r, v)
	return r.Error()
}

// UnmarshalEasyJSON supports easyjson.Unmarshaler interface
func (v *DHGroup) UnmarshalEasyJSON(l *jlexer.Lexer) {
	easyjson27a961dcDecodeGitRonaksoftComRiverSdkSdkPrime3(l, v)
}
