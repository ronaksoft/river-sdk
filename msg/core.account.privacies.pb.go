...
// source: core.account.privacies.proto
...
func (m *AccountPrivacyAllowUsers) MarshalTo(dAtA []byte) (int, error) {
	...
	if len(m.UserIDs) > 0 {
		for _, num := range m.UserIDs {
			dAtA[i] = 0x8
			i++
			i = encodeVarintCoreAccountPrivacies(dAtA, i, uint64(num))
		}
	}
	return i, nil
}
...
func encodeVarintCoreAccountPrivacies(dAtA []byte, offset int, v uint64) int {
	for v >= 1<<7 {
...
func (m *AccountPrivacyAllowUsers) Size() (n int) {
	...
	if len(m.UserIDs) > 0 {
		for _, e := range m.UserIDs {
			n += 1 + sovCoreAccountPrivacies(uint64(e))
		}
	}
	return n
}
...
func sovCoreAccountPrivacies(x uint64) (n int) {
	for {
		n++
		x >>= 7
		if x == 0 {
...
func sozCoreAccountPrivacies(x uint64) (n int) {
	return sovCoreAccountPrivacies(uint64((x << 1) ^ uint64((int64(x) >> 63))))
}
func (m *AccountPrivacyAllowAll) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowCoreAccountPrivacies
			}
			if iNdEx >= l {
		...
		default:
			iNdEx = preIndex
			skippy, err := skipCoreAccountPrivacies(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
				return ErrInvalidLengthCoreAccountPrivacies
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
...
func skipCoreAccountPrivacies(dAtA []byte) (n int, err error) {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return 0, ErrIntOverflowCoreAccountPrivacies
			}
			if iNdEx >= l {
		...
		case 2:
			...
			iNdEx += length
			if length < 0 {
				return 0, ErrInvalidLengthCoreAccountPrivacies
			}
			return iNdEx, nil
		case 3:
			for {
				...
				next, err := skipCoreAccountPrivacies(dAtA[start:])
				if err != nil {
					return 0, err
				}
				iNdEx = start + next
