...
func (r *gif) savedGifs(e *rony.MessageEnvelope) {
	...
	if err != nil {
		logs.Error("GIFModule couldn't unmarshal savedGifs", zap.Error(err))
		return
	}
	...
	for _, d := range u.Docs {
		...
		if err != nil {
			logs.Warn("GIFModule got error on applying SavedGifs (Save File)", zap.Error(err))
		}
		if !repo.Gifs.IsSaved(d.Doc.ClusterID, d.Doc.ID) {
			err = repo.Gifs.Save(d)
			if err != nil {
				logs.Warn("GIFModule got error on applying SavedGifs (Save Gif)", zap.Error(err))
			}
			err = repo.Gifs.UpdateLastAccess(d.Doc.ClusterID, d.Doc.ID, accessTime)
			if err != nil {
				logs.Warn("GIFModule got error on applying SavedGifs (Update Access Time)", zap.Error(err))
			}
		}
	}
	...
	if err != nil {
		logs.Warn("GIFModule got error on saving GifHash", zap.Error(err))
	}
