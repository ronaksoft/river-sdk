...
func (ctrl *CtrlNetwork) Send(msgEnvelope *msg.MessageEnvelope) error {
	...
	if msgEnvelope.Constructor != msg.C_AuthRecall {
		...
		_Log.Debug("Crc32 of MessageKey & RequestID",
...
func (ctrl *CtrlNetwork) onConnect() {
	...
	if err != nil {
		_Log.Error("onConnect() AuthRecall", zap.Error(err))
	}
...
func (ctrl *CtrlNetwork) receiver() {
	...
	for {
		...
		if err != nil {
			_Log.Error("CtrlNetwork::receiver() failed to unmarshal to ProtoMessage", zap.Error(err))
			continue
		}

		if res.AuthID != ctrl.actor.GetAuthID() {
			_Log.Error("Received unmatched AuthID ", zap.Int64("Server.AutID", res.AuthID), zap.Int64("Actor.AuthID", ctrl.actor.GetAuthID()))
		}
		...
		if res.AuthID == 0 {
			...
			if err != nil {
				_Log.Error("CtrlNetwork::receiver() failed to unmarshal to MessageEnvelope", zap.Error(err))
				continue
			}
			...
			if err != nil {
				_Log.Error("CtrlNetwork::receiver() failed to domain.Decrypt()", zap.Error(err))
				continue
			}
			receivedEncryptedPayload := new(msg.ProtoEncryptedPayload)
			err = receivedEncryptedPayload.Unmarshal(decryptedBytes)
			if err != nil {
				_Log.Error("CtrlNetwork::receiver() failed to unmarshal to ProtoEncryptedPayload", zap.Error(err))
				continue
			}
