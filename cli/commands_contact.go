...
var ContactImport = &ishell.Cmd{
	...
	Func: func(c *ishell.Context) {
		...
		if reqID, err := _SDK.ExecuteCommand(msg.C_ContactsImport, reqBytes, reqDelegate, false); err != nil {
...
var ContactGet = &ishell.Cmd{
	...
	Func: func(c *ishell.Context) {
		...
		reqBytes, _ := req.Marshal()
		reqDelegate := new(RequestDelegate)
		if reqID, err := _SDK.ExecuteCommand(msg.C_ContactsGet, reqBytes, reqDelegate, false); err != nil {
			_Log.Debug(err.Error())
		} else {
			reqDelegate.RequestID = reqID
		}
