// Code generated by protoc-gen-gogo. DO NOT EDIT.
// source: chat.password.algorithms.proto

package msg

import (
	fmt "fmt"
	proto "github.com/gogo/protobuf/proto"
	io "io"
	math "math"
	math_bits "math/bits"
)

// Reference imports to suppress errors if they are not otherwise used.
var _ = proto.Marshal
var _ = fmt.Errorf
var _ = math.Inf

// This is a compile-time assertion to ensure that this generated file
// is compatible with the proto package it is being compiled against.
// A compilation error at this line likely means your copy of the
// proto package needs to be updated.
const _ = proto.GoGoProtoPackageIsVersion3 // please upgrade the proto package

type PasswordAlgorithmVer6A struct {
	Salt1 []byte `protobuf:"bytes,1,opt,name=Salt1,proto3" json:"Salt1,omitempty"`
	Salt2 []byte `protobuf:"bytes,2,opt,name=Salt2,proto3" json:"Salt2,omitempty"`
	G     int32  `protobuf:"varint,3,opt,name=G,proto3" json:"G,omitempty"`
	P     []byte `protobuf:"bytes,4,opt,name=P,proto3" json:"P,omitempty"`
}

func (m *PasswordAlgorithmVer6A) Reset()         { *m = PasswordAlgorithmVer6A{} }
func (m *PasswordAlgorithmVer6A) String() string { return proto.CompactTextString(m) }
func (*PasswordAlgorithmVer6A) ProtoMessage()    {}
func (*PasswordAlgorithmVer6A) Descriptor() ([]byte, []int) {
	return fileDescriptor_df41f1cafa3a3ce5, []int{0}
}
func (m *PasswordAlgorithmVer6A) XXX_Unmarshal(b []byte) error {
	return m.Unmarshal(b)
}
func (m *PasswordAlgorithmVer6A) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	if deterministic {
		return xxx_messageInfo_PasswordAlgorithmVer6A.Marshal(b, m, deterministic)
	} else {
		b = b[:cap(b)]
		n, err := m.MarshalToSizedBuffer(b)
		if err != nil {
			return nil, err
		}
		return b[:n], nil
	}
}
func (m *PasswordAlgorithmVer6A) XXX_Merge(src proto.Message) {
	xxx_messageInfo_PasswordAlgorithmVer6A.Merge(m, src)
}
func (m *PasswordAlgorithmVer6A) XXX_Size() int {
	return m.Size()
}
func (m *PasswordAlgorithmVer6A) XXX_DiscardUnknown() {
	xxx_messageInfo_PasswordAlgorithmVer6A.DiscardUnknown(m)
}

var xxx_messageInfo_PasswordAlgorithmVer6A proto.InternalMessageInfo

func (m *PasswordAlgorithmVer6A) GetSalt1() []byte {
	if m != nil {
		return m.Salt1
	}
	return nil
}

func (m *PasswordAlgorithmVer6A) GetSalt2() []byte {
	if m != nil {
		return m.Salt2
	}
	return nil
}

func (m *PasswordAlgorithmVer6A) GetG() int32 {
	if m != nil {
		return m.G
	}
	return 0
}

func (m *PasswordAlgorithmVer6A) GetP() []byte {
	if m != nil {
		return m.P
	}
	return nil
}

func init() {
	proto.RegisterType((*PasswordAlgorithmVer6A)(nil), "msg.PasswordAlgorithmVer6A")
}

func init() { proto.RegisterFile("chat.password.algorithms.proto", fileDescriptor_df41f1cafa3a3ce5) }

var fileDescriptor_df41f1cafa3a3ce5 = []byte{
	// 164 bytes of a gzipped FileDescriptorProto
	,
}

func (m *PasswordAlgorithmVer6A) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBuffer(dAtA[:size])
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *PasswordAlgorithmVer6A) MarshalTo(dAtA []byte) (int, error) {
	size := m.Size()
	return m.MarshalToSizedBuffer(dAtA[:size])
}

func (m *PasswordAlgorithmVer6A) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	if len(m.P) > 0 {
		i -= len(m.P)
		copy(dAtA[i:], m.P)
		i = encodeVarintChatPasswordAlgorithms(dAtA, i, uint64(len(m.P)))
		i--
		dAtA[i] = 0x22
	}
	if m.G != 0 {
		i = encodeVarintChatPasswordAlgorithms(dAtA, i, uint64(m.G))
		i--
		dAtA[i] = 0x18
	}
	if len(m.Salt2) > 0 {
		i -= len(m.Salt2)
		copy(dAtA[i:], m.Salt2)
		i = encodeVarintChatPasswordAlgorithms(dAtA, i, uint64(len(m.Salt2)))
		i--
		dAtA[i] = 0x12
	}
	if len(m.Salt1) > 0 {
		i -= len(m.Salt1)
		copy(dAtA[i:], m.Salt1)
		i = encodeVarintChatPasswordAlgorithms(dAtA, i, uint64(len(m.Salt1)))
		i--
		dAtA[i] = 0xa
	}
	return len(dAtA) - i, nil
}

func encodeVarintChatPasswordAlgorithms(dAtA []byte, offset int, v uint64) int {
	offset -= sovChatPasswordAlgorithms(v)
	base := offset
	for v >= 1<<7 {
		dAtA[offset] = uint8(v&0x7f | 0x80)
		v >>= 7
		offset++
	}
	dAtA[offset] = uint8(v)
	return base
}
func (m *PasswordAlgorithmVer6A) Size() (n int) {
	if m == nil {
		return 0
	}
	var l int
	_ = l
	l = len(m.Salt1)
	if l > 0 {
		n += 1 + l + sovChatPasswordAlgorithms(uint64(l))
	}
	l = len(m.Salt2)
	if l > 0 {
		n += 1 + l + sovChatPasswordAlgorithms(uint64(l))
	}
	if m.G != 0 {
		n += 1 + sovChatPasswordAlgorithms(uint64(m.G))
	}
	l = len(m.P)
	if l > 0 {
		n += 1 + l + sovChatPasswordAlgorithms(uint64(l))
	}
	return n
}

func sovChatPasswordAlgorithms(x uint64) (n int) {
	return (math_bits.Len64(x|1) + 6) / 7
}
func sozChatPasswordAlgorithms(x uint64) (n int) {
	return sovChatPasswordAlgorithms(uint64((x << 1) ^ uint64((int64(x) >> 63))))
}
func (m *PasswordAlgorithmVer6A) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowChatPasswordAlgorithms
			}
			if iNdEx >= l {
				return io.ErrUnexpectedEOF
			}
			b := dAtA[iNdEx]
			iNdEx++
			wire |= uint64(b&0x7F) << shift
			if b < 0x80 {
				break
			}
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: PasswordAlgorithmVer6A: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: PasswordAlgorithmVer6A: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Salt1", wireType)
			}
			var byteLen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowChatPasswordAlgorithms
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				byteLen |= int(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			if byteLen < 0 {
				return ErrInvalidLengthChatPasswordAlgorithms
			}
			postIndex := iNdEx + byteLen
			if postIndex < 0 {
				return ErrInvalidLengthChatPasswordAlgorithms
			}
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			m.Salt1 = append(m.Salt1[:0], dAtA[iNdEx:postIndex]...)
			if m.Salt1 == nil {
				m.Salt1 = []byte{}
			}
			iNdEx = postIndex
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Salt2", wireType)
			}
			var byteLen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowChatPasswordAlgorithms
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				byteLen |= int(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			if byteLen < 0 {
				return ErrInvalidLengthChatPasswordAlgorithms
			}
			postIndex := iNdEx + byteLen
			if postIndex < 0 {
				return ErrInvalidLengthChatPasswordAlgorithms
			}
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			m.Salt2 = append(m.Salt2[:0], dAtA[iNdEx:postIndex]...)
			if m.Salt2 == nil {
				m.Salt2 = []byte{}
			}
			iNdEx = postIndex
		case 3:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field G", wireType)
			}
			m.G = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowChatPasswordAlgorithms
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				m.G |= int32(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		case 4:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field P", wireType)
			}
			var byteLen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowChatPasswordAlgorithms
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				byteLen |= int(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			if byteLen < 0 {
				return ErrInvalidLengthChatPasswordAlgorithms
			}
			postIndex := iNdEx + byteLen
			if postIndex < 0 {
				return ErrInvalidLengthChatPasswordAlgorithms
			}
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			m.P = append(m.P[:0], dAtA[iNdEx:postIndex]...)
			if m.P == nil {
				m.P = []byte{}
			}
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skipChatPasswordAlgorithms(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
				return ErrInvalidLengthChatPasswordAlgorithms
			}
			if (iNdEx + skippy) < 0 {
				return ErrInvalidLengthChatPasswordAlgorithms
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}
func skipChatPasswordAlgorithms(dAtA []byte) (n int, err error) {
	l := len(dAtA)
	iNdEx := 0
	depth := 0
	for iNdEx < l {
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return 0, ErrIntOverflowChatPasswordAlgorithms
			}
			if iNdEx >= l {
				return 0, io.ErrUnexpectedEOF
			}
			b := dAtA[iNdEx]
			iNdEx++
			wire |= (uint64(b) & 0x7F) << shift
			if b < 0x80 {
				break
			}
		}
		wireType := int(wire & 0x7)
		switch wireType {
		case 0:
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return 0, ErrIntOverflowChatPasswordAlgorithms
				}
				if iNdEx >= l {
					return 0, io.ErrUnexpectedEOF
				}
				iNdEx++
				if dAtA[iNdEx-1] < 0x80 {
					break
				}
			}
		case 1:
			iNdEx += 8
		case 2:
			var length int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return 0, ErrIntOverflowChatPasswordAlgorithms
				}
				if iNdEx >= l {
					return 0, io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				length |= (int(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			if length < 0 {
				return 0, ErrInvalidLengthChatPasswordAlgorithms
			}
			iNdEx += length
		case 3:
			depth++
		case 4:
			if depth == 0 {
				return 0, ErrUnexpectedEndOfGroupChatPasswordAlgorithms
			}
			depth--
		case 5:
			iNdEx += 4
		default:
			return 0, fmt.Errorf("proto: illegal wireType %d", wireType)
		}
		if iNdEx < 0 {
			return 0, ErrInvalidLengthChatPasswordAlgorithms
		}
		if depth == 0 {
			return iNdEx, nil
		}
	}
	return 0, io.ErrUnexpectedEOF
}

var (
	ErrInvalidLengthChatPasswordAlgorithms        = fmt.Errorf("proto: negative length found during unmarshaling")
	ErrIntOverflowChatPasswordAlgorithms          = fmt.Errorf("proto: integer overflow")
	ErrUnexpectedEndOfGroupChatPasswordAlgorithms = fmt.Errorf("proto: unexpected end of group")
)
